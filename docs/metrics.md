## Способ получения значений метрик

Метрики кода были рассчитаны с помощью инструмента radon при помощи следующих команд:
...
Из выводов были выбраны только содержательные части и оформлены в виде сводных таблиц для дальнейшего анализа.
...
* Проблема: Заметим, что в `server.py` 145 строк кода, в то время как во всех файлах 261 строка. То есть больше половины строк сосредоточено в одном файле, из-за чего он может быть перегружен.
...
* Как улучшить: Проверку авторизации можно было бы вынести в отдельный файл, так как он представляет собой отдельный логический блок, при увеличении размера проекта он также будет расширяться (за счет обратки OAuth-токенов и session_id). Также можно вынести логику работы с базой данных.

3. **Избыточность кода в моделях**

* Метрики: Метрики Холстеда

* Проблема: Модельные классы имеют нулевые значения метрик Холстеда, что может указывать на дублирование кода без логики.

* Подтверждение гипотезы: Действительно, в файле `models.py` представлены классы-модели, при этом не используется наследование.

* Как улучшить: Использовать наследование, абстрактные классы для общей логики, чтобы уменьшить дублирование кода

4. **Отсутствие комментариев**

* Метрики: Comments, Single Comments

* Проблема: всего 1 комментарий на весь проект. Код становится трудночитаемым, сложные моменты не поясняются.

* Подтверждение гипотезы: Действительно, написанный код не ориентирован на легкое понимание для других разработчиков, в нем отсутствуют комментарии.

* Как улучшить: Использовать комментарии в сложных частях кода, использовать docstrings для классов и хэндлеров запросов.